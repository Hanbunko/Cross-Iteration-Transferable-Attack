...
def imshow(img, pth = "default", ttl = "default"):
...
def main():
    ...
    early_model = torch.load(f'{src_dic}/1000.pth')
    ...
    for batch_idx, [images, labels] in tqdm.tqdm(enumerate(test_loader)):
        ...
        pred = final_model(ae)
        po = final_model(images)
        succ += (labels.numpy() == pred.argmax(dim=1).detach().cpu().numpy()).sum()
        tar += (pred.argmax(dim=1).detach().cpu().numpy() == new_labels.cpu()).sum()
        total += labels.shape[0]

        ctwo = F.softmax(pred, dim = 1)
        ctwo,_ = torch.max(ctwo, dim = 1)
        cone = F.softmax(po, dim = 1)
        cone,_ = torch.max(cone, dim = 1)

        # torch.save(ae[1], f'sam_mim/{batch_idx}.pt')
        loc = ['airplane', 'automobile', 'bird', 'cat','deer', 'dog', 'frog', 'horse', 'ship', 'truck']
        ptwo = pred.argmax(dim=1).detach().cpu().numpy()
        pone = po.argmax(dim=1).detach().cpu().numpy()
        imshow(torchvision.utils.make_grid(images[1]), pth = f'sam_mim/{batch_idx}_ori', ttl = f"Original Prediction: {loc[pone[1]]} with confidence {cone[1]*100:.2f}%")
        imshow(torchvision.utils.make_grid(ae[1]), pth = f'sam_mim/{batch_idx}', ttl = f"Perturbed Prediction: {loc[ptwo[1]]} with confidence {ctwo[1]*100:.2f}%")
    if targeted_class >= 0:
        tasr = 100 * (tar / total)
        tasrs.append(tasr)
