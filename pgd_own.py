...
# Main Evaluation
# ===============
def plot_confusion_matrix(predictions, labels, rou, class_names=None):
    # Create the cmp directory if it doesn't exist
    os.makedirs('./exp_00_plot', exist_ok=True)
    ...
    plt.tight_layout()
    plt.savefig(f'./vnites/confusion_matrix_{rou}_{tpy}.png')
    plt.close()
...
def main():
    ...
    earou = 100
    rounds = list(range(earou, earou+100, 100))
    for r in rounds:
        ...
        for batch_idx, [images, labels] in tqdm.tqdm(enumerate(test_loader)):
            ...
            lae = pgdleft(final_model, criterion, images, labels, target_class=targeted_class, epsilon=8/255., lr=2/255., num_iters=10)
            rae = pgdright(early_model, criterion, images, labels, target_class=targeted_class, epsilon=8/255., lr=2/255., num_iters=10)

            ae = images + (lae - images) + (rae - images)
            ...
            # imshow(torchvision.utils.make_grid(ae[1]), pth = f'sam_pgd/right_{batch_idx}')
        if targeted_class >= 0:
            tasr = 100 * (tar / total)
            tasrs.append(tasr)
