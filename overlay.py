...
class EnsembleMIFGSM:
    ...
    def __call__(self, images, labels, targeted=False):
        ...
        for _ in range(self.steps):
            ...
            ensemble_grad = ensemble_grad / len(picked)
...
def main():
    ...
    for r in rounds:
        ...
        if method == 0:
            ensemble_models = load_ensembles(early_model, num_ensemble_models, "at_models", num_cli = noc)
        elif method == 1:
            ensemble_models = load_ensembles(early_model, num_ensemble_models, "ens_models", num_cli = noc)
        elif method == 2:
            ensemble_models = load_ensembles(early_model, num_ensemble_models, "ensp_models", num_cli = noc)
        else:
            ensemble_models = load_ensembles(early_model, num_ensemble_models, mod_dir, num_cli = noc)
        ...
        vvv = [0]*(num_ensemble_models+1)
        aaa = [0]*(num_ensemble_models+1)
        ...
        for batch_idx, [images, labels] in tqdm.tqdm(enumerate(test_loader),
            ...
            for mod in ensemble_models:
                this_att = torchattacks.MIFGSM(mod, eps=epsilon, alpha=2/255, steps=noiters)
                this_ae = this_att(images, labels.to(device))
                ensemble_ae = ensemble_ae + (this_ae - images)
                delta = ensemble_ae - images
                delta = torch.clamp(delta.detach(), -8/255, 8/255)
                ensemble_ae = torch.clamp(images + delta, 0, 1)
