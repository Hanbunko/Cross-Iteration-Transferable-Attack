...
def main():
    targeted_class = 4  # Target class for the attack
    # 0 = PGD, 1 = MIM, 2 = DIFGSM, 3 = BSR, 4 = VNIFGSM
    att_typ = 4

    ckpt_dir = 'mod_imb'; set_seed(2025)
    device = best_device()
    ...
    for r in rounds:
        ...
        if att_typ == 0:
            criterion = nn.CrossEntropyLoss()
            ae = pgd(early_model, criterion, data, target, target_class=targeted_class, epsilon=8/255., lr=2/255., num_iters=10)
        elif att_typ == 1:
            atk = torchattacks.MIFGSM(early_model, eps = 8/255, alpha = 2/255, steps = 10)
            ae = atk(data, target)
        elif att_typ == 2:
            atk = torchattacks.DIFGSM(early_model, eps = 8/255, alpha = 2/255, steps = 10)
            ae = atk(data, target)
        elif att_typ == 3:
            ae = bsr_attack(early_model, data, target, epsilon = 8/255)
        elif att_typ == 4:
            atk = torchattacks.VNIFGSM(early_model, eps = 8/255, alpha = 2/255, steps = 10)
            if targeted_class >= 0:
                atk.set_mode_targeted_by_label()
                new_labels = len(target)* [targeted_class]  # Targeting class
                new_labels = torch.tensor(new_labels, device=device)
                ae = atk(data, new_labels)
            else:
                ae = atk(data, target)
        else:
            print("Attack Type DNE")
            break
        with torch.no_grad():
            if targeted_class >= 0:
                tasr, asr, all_predictions, all_labels = evaluate_asr(final_model, ae, target, type=targeted_class)
                tasrs.append(tasr)
            else:
                tasr, asr, all_predictions, all_labels = evaluate_asr(final_model, ae, target, type=targeted_class)
            # plot_confusion_matrix(np.array(all_predictions), np.array(all_labels), rou = r, tpy = -2)
            asrs.append(asr)
    print(f"TASR List: {[round(tasr, 2) for tasr in tasrs]}")
    print(f"ASR List: {[round(asr, 2) for asr in asrs]}")
    print("="*60)
