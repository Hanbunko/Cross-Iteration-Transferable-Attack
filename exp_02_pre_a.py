...
def create_diversified_ensemble_cifar100(early_model, train_loader, num_models=10):
    ...
    for tc in range(num_models):
        ...
        model.eval()
        # Save model
        os.makedirs(f'./exp_02_models_{leak_round}', exist_ok=True)
        torch.save(model, f'./exp_02_models_{leak_round}/{tc+1}.pth')
...
def main():
    ...
    num_ensemble_models = 20  # Number of models in ensemble
