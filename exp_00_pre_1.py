...
def create_diversified_ensemble_cifar100(early_model, train_loader, num_models=10):
    ...
    for tc in range(num_models):
        ...
        for images, _ in train_loader:
            ...
            with torch.no_grad():
                soft_labels = ensemble_models[0](images)
            ...
            # Generate adversarial examples targeting random CIFAR-10 classes
        ...
        model.eval()
        # Save model
        os.makedirs(f'./exp_00_models_{leak_round}', exist_ok=True)
        torch.save(model, f'./exp_00_models_{leak_round}/{tc+1}.pth')
